...
if __name__ == "__main__":
    ...
    num_epochs = 30  # 训练轮次
    num_classes = 4   # 模型使用的数据集和网络最后一层输出参数
    lr = 1e-3  # 学习率
    save_dir = 'model_result'


    train_dataloader = DataLoader(VideoDataset(dataset_path='data/gesture-data', images_path='train', clip_len=16), batch_size=16, shuffle=True, num_workers=2)
    val_dataloader = DataLoader(VideoDataset(dataset_path='data/gesture-data', images_path='val', clip_len=16), batch_size=16, num_workers=2)
    test_dataloader = DataLoader(VideoDataset(dataset_path='data/gesture-data', images_path='test', clip_len=16), batch_size=16, num_workers=2)
